...
function vibeKeyword(insights: RoomInsight[], stats: RoomStats): string {
  const texts = insights.map((i) => i.detail.toLowerCase() + " " + i.title.toLowerCase()).join(" ");
  if (texts.includes("focus") || texts.includes("quiet") || texts.includes("typing"))
    return "deep focus era";
  if (texts.includes("talk") || texts.includes("chat") || texts.includes("social"))
    return "yapping session";
  if (texts.includes("walk") || texts.includes("moving") || texts.includes("entry"))
    return "main character walks";
  if (texts.includes("phone") || texts.includes("device"))
    return "doom scrolling";
  if (texts.includes("eating") || texts.includes("food"))
    return "munch o'clock";
  if (stats.activePeople === 0) return "NPC hours";
  if (stats.activePeople > 15) return "absolute chaos";
  return "just vibing";
}
...
export function BuildingVibePanel({
  ...
  const vibe = vibeKeyword(insights, stats);
  ...
  return (
    <GlassPanel className="flex h-full flex-col border-none bg-transparent shadow-none backdrop-blur-none backdrop-saturate-100 p-6">
      {/* Header */}
      <div className="mb-6">
        <h2 className="text-lg font-bold tracking-tight text-white">Building pulse</h2>
        <p className="mt-1 text-xs text-white/40">Real-time vibe check</p>
      </div>
      ...
      <motion.div
        ...
        className="mb-6 rounded-2xl border border-white/[0.08] bg-gradient-to-br from-white/[0.04] to-white/[0.01] p-5"
      >
        <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-white/30">
          Today's vibe
        </p>
        <p className="mt-2 text-6xl font-bold leading-tight text-white" style={{ fontFamily: "var(--font-caveat)" }}>
          {vibe}
        </p>
      </motion.div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-4">
          ...
          <p className="text-[10px] text-white/30">{stats.ambientDb > 0 ? `~${stats.ambientDb} dB` : "no data"}</p>
        </div>

        {/* Hot zone */}
        <div className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-4">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-white/35">Hot zone</p>
      ...
      <div className="mt-5 flex-1 overflow-y-auto">
        ...
        <ul className="flex flex-col gap-1.5">
          {insights.slice(0, 6).map((ins) => (
            ...
            >
              <p className="text-[11px] font-medium text-white/75">{ins.title}</p>
